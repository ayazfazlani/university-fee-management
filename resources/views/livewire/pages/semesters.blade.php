            ...
            <div class="modal-dialog modal-dialog-centered">
                <form wire:submit.prevent="{{ $selectedSemester ? 'update' : 'store' }}" class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select wire:model="sessionId" class="form-control">
                    ...
                    <div class="modal-footer gap-2">
                        ...
                        <button type="submit"
                            class="btn btn-primary">{{ $selectedSemester ? 'Update' : 'Save' }}</button>
                    </div>
                </form>
            </div>
        </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a class="btn btn-sm btn-info"
                                                href="{{ route('class-members', ['semester' => $semester->id, 'section' => $semester->section->id]) }}">View
                                                class</a>
                                            {{-- <button class="btn btn-sm btn-info"
                                            wire:click="view({{ $semester->id }})">View class</button> --}}
                                            <button class="btn btn-sm btn-primary"
                                                wire:click="edit({{ $semester->id }})">Edit</button>
                                            <button class="btn btn-sm btn-danger"
                                                wire:click="delete({{ $semester->id }})">Delete</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
